hclust_plot_any <- function(){
    ggplot() +
        geom_segment(data = hcd_data[[3]]$segments,
                     aes(x = x,
                         y = y,
                         xend = xend,
                         yend = yend)) +
        ...
        #           size = 10) +
        scale_y_reverse(limits = c(160,-54))+
        ...
        scale_color_manual(values = c('#a6cee3',
                                      ...
                                      '#b2df8a',
                                      '#33a02c'))+
        labs(title = "Timing of Any Efforts",
             color = " ")  -> p_any
    return(p_any)
}
